...
export function GridPattern({ yOffset = 0, interactive = false, ...props }) {
  ...
  return (
    <chakra.svg ref={ref} aria-hidden="true" {...props}>
      ...
      {/* Plain <svg>: v3's chakra() factory swallows `x` and `y` on SVG
          elements. Same fix, and same reason, as in ArrowPattern. */}
      <svg x="50%" y={yOffset} strokeWidth="0" overflow="visible">
        ...
        {hoveredBlocks.map(block => (
          <Block
            ...
            x={block[0]}
            y={block[1]}
            // See ArrowPattern: the same one-shot flash, as a theme keyframe.
