...
export function ArrowPattern({
  ...
  useEffect(() => {
    ...
    function onMouseMove(event: MouseEvent) {
      ...
      // Invert cellToPixel: rotate into the lattice axes and floor. The
      // lattice is generated by (HALF, HALF) and (HALF, -HALF), so the two
      // axes are the sum and the difference of the coordinates.
      const scale = size / TILE;
      const u = (x / scale + y / scale) / TILE;
      const v = (x / scale - y / scale) / TILE;
      const i = Math.floor(u + v);
      const j = Math.floor(u - v);
