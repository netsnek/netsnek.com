...
export function StylizedImage({ shape = 0, ...props }) {
  let id = useId();
  let { width, height, path } = shapes[shape];

  return (
    <Box position="relative" filter="grayscale" w="full">
      <AspectRatio ratio={719 / 680}>
        <svg viewBox={`0 0 ${width} ${height}`} fill="none">
          <g clipPath={`url(#${id}-clip)`}>
            <g style={{ transition: 'transform 0.5s', transformOrigin: 'center' }}>
